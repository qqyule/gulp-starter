...
const baleJs = () =>
	src([configUrl.file.js, '!' + configUrl.file.libs])
...
const baleCopy = () =>
	src([configUrl.file.fonts, configUrl.file.libs], { base: baseUrl }).pipe(
		dest(distUrl)
	)

const file = () =>
	src([configUrl.file.tpl, '!' + configUrl.file.tpl_include])
		.pipe(
			fileinclude({
				prefix: '@@', //变量前缀 @@include
...
const reload = () => src(configUrl.file.html).pipe(connect.reload())
...
exports.build = series(
	clean,
	parallel(baleCss, baleJs, baleImages, baleCopy),
	baleHtml
)
